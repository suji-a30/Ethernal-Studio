import { Search, Moon, Bell, Undo, Redo, Upload, Download } from 'lucide-react';
    ...
    <header className="h-[72px] flex-shrink-0 bg-white/70 backdrop-blur-xl flex items-center justify-between px-8 border-b border-white shadow-sm sticky top-0 z-20">
      ...
      {currentView === 'editor' ? (
        <div className="flex items-center gap-3">
          <button className="w-10 h-10 flex items-center justify-center text-slate-700 hover:bg-slate-100 rounded-full transition-colors">
            <Undo size={18} />
          </button>
          <button className="w-10 h-10 flex items-center justify-center text-slate-700 hover:bg-slate-100 rounded-full transition-colors">
            <Redo size={18} />
          ...
          <button className="h-10 px-5 rounded-full bg-[#9bdaff] text-[#004d6a] hover:bg-opacity-80 transition-colors font-medium text-[15px] flex items-center gap-2">
            <Upload size={16} /> Upload
          </button>
          <button className="h-10 px-6 rounded-full bg-[#a052e6] text-white hover:bg-[#9241da] font-medium text-[15px] flex items-center gap-2 shadow-sm">
